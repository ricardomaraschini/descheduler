...
// Indexer is a function that receives an object and returns a key. This
// function is used to generate a key for a map based on the object.
type Indexer[K comparable, O any] func(O) K
...
// Min receives a minimum value and a list of values. Returns a list of values
// where each value is greater than the minimum value.
func Min[K comparable, N Number, V ~map[K]N](values []V, limit N) []V {
	result := []V{}
	for _, item := range values {
		newItem := V{}
		for key, value := range item {
			newItem[key] = N(
				math.Max(float64(value), float64(limit)),
			)
		}
