...
type Comparer[V any, N Number] func(V, V) N
...
// the map values one at a time. The returned Classifier will return true if the
// provided Comparer function returns a value less than 0 for any of the values.
func ForMapAny[K, I comparable, V any, M ~map[I]V, N Number](cmp Comparer[V, N]) Classifier[K, M] {
	return func(_ K, usages, limits M) bool {
		for idx, usage := range usages {
			if limit, ok := limits[idx]; ok {
				if cmp(usage, limit) < 0 {
